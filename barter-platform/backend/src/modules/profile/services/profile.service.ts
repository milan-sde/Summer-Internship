...
export class ProfileService {
  ...
  async createProfile(
    ...
    if (user.role === "INFLUENCER") {
      ...
      const hasInstagram =
        instagram && instagram.username && instagram.followers !== undefined;
      const hasYoutube =
        youtube && youtube.username && youtube.followers !== undefined;
      const hasTwitter =
        twitter && twitter.username && twitter.followers !== undefined;

      if (!hasInstagram && !hasYoutube && !hasTwitter) {
        throw new ValidationError(
          "At least one social media platform must be fully filled with both username and followers",
        );
      }
      ...
      if (
        instagram &&
        ((instagram.username && instagram.followers === undefined) ||
          (!instagram.username && instagram.followers !== undefined))
      ) {
        throw new ValidationError(
          "Instagram username and followers must both be filled if one is provided",
        );
      }
      if (
        youtube &&
        ((youtube.username && youtube.followers === undefined) ||
          (!youtube.username && youtube.followers !== undefined))
      ) {
        throw new ValidationError(
          "YouTube username and followers must both be filled if one is provided",
        );
      }
      if (
        twitter &&
        ((twitter.username && twitter.followers === undefined) ||
          (!twitter.username && twitter.followers !== undefined))
      ) {
        throw new ValidationError(
          "Twitter username and followers must both be filled if one is provided",
        );
      }
      ...
      if (!data.firstName || !data.lastName) {
        throw new ValidationError(
          "First name and last name are required for brands",
        );
      }
      if (!data.phoneNumber) {
  ...
  async getPublicProfile(instagramHandle: string): Promise<any> {
    const profile =
      await this.profileRepository.findByInstagramHandle(instagramHandle);
  ...
  async updateProfile(
    ...
    if (
      data.instagramHandle &&
      data.instagramHandle !== profile.instagramHandle
    ) {
      const handleTaken = await this.profileRepository.findByInstagramHandle(
        data.instagramHandle,
      );
  ...
  private toResponseDto(profile: any): ProfileResponseDto {
    ...
    return {
      ...
      instagram:
        profile.instagram && profile.instagram.instagramId
          ? {
              instagramId: profile.instagram.instagramId,
              username: profile.instagram.username,
              followersCount: profile.instagram.followersCount,
              mediaCount: profile.instagram.mediaCount,
              profilePicture: profile.instagram.profilePicture,
              connectedAt: profile.instagram.connectedAt,
            }
          : undefined,
  ...
  /**
   * Retrieves the unified influencer profile object containing profile details,
   * cached Instagram feed items, and local portfolio items.
   * Runs queries concurrently via Promise.all to minimize latency.
   *
   * @param influencerId The user ID of the influencer
   */
  async getInfluencerFullProfile(
    influencerId: string,
    ...
    if (!mongoose.Types.ObjectId.isValid(influencerId)) {
      throw new ValidationError("Invalid influencer ID format");
    }

    // 1. Fetch Profile details
    const profile = await this.profileRepository.findByUserId(influencerId);
    if (!profile) {
      throw new NotFoundError("Influencer Profile");
    }

    // Security & Logic: Ensure the profile belongs to an influencer
    if (profile.role !== "INFLUENCER") {
      throw new ValidationError("The requested profile is not an influencer");
    }
    ...
    const portfolioMediaPromise = PortfolioMedia.find({
      userId: profile.userId,
    }).sort({ createdAt: -1 });

    const [instagramMedia, portfolioMedia] = await Promise.all([
      instagramMediaPromise,

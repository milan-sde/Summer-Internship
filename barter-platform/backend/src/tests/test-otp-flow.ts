...
async function runOtpTests() {
  ...
  try {
    ...
    const otp1 = await otpService.generateAndStoreOtp(testEmail);
    ...
    const storedOtp = await otpRepo.findValid(testEmail, "email_verification");
    if (!storedOtp) {
      throw new Error("OTP should exist in database");
    }
    console.log("   ✅ OTP persisted in database");
    ...
    try {
      await otpService.resendOtp(testEmail);
    ...
    await otpService.deleteOtp(testEmail);
    const otp2 = await otpService.generateAndStoreOtp(testEmail);
    console.log(`   ✅ OTP generated after bypassing cooldown: ${otp2}`);

    // ----------------------------------------------------
    // TEST 4: Verification & Reuse Prevention
    // ----------------------------------------------------
    console.log("\n   TEST 4: OTP Verification & Reuse Prevention...");
    await otpService.deleteOtp(testEmail);
    const otp3 = await otpService.generateAndStoreOtp(testEmail);
    
    const isValid = await otpService.verifyOtp(testEmail, otp3);
    if (!isValid) {
    ...
    try {
      await otpService.verifyOtp(testEmail, otp3);
    ...
    // ----------------------------------------------------
    // TEST 5: SMTP Failure DB/Store Consistency
    // ----------------------------------------------------
    console.log("\n   TEST 5: SMTP Failure Handling...");
    ...
    try {
      ...
      const hasOtp = await otpService.hasValidOtp(testRegistrationEmail);
      if (hasOtp) {
    ...
    // ----------------------------------------------------
    // TEST 6: End-to-End Success Path
    // ----------------------------------------------------
    console.log("\n   TEST 6: End-to-End Registration & Verification Flow...");
